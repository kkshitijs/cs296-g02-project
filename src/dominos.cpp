...
void dominos_t::see_saw_create(float x_pos, float y_pos, float plank_length, float plank_breadth)
{
  ...
  <li> Balls at the bottom level and on platform above pulley system near the bottom</li>
  <li> The pulley system near the bottom
  		<ul> <li> An open box</li>
  			<li> A domino</li> 
  		</ul></li>	
  <li> Dominos near the bottom </li>
  ...
  dominos_t::dominos_t()
  {
    ...
    ///<br>
    ///7.<b> 5 balls at the bottom level and 2 on platform above the pulley system near the bottom</b>
    {
    	/// 7.a) <b>balls at bottom level</b>
	...
	b2FixtureDef ballfd;/// define a fixture ballfd
      ...
      /**<i> for 1 body </i>: */
    		b2BodyDef ballbd;/// b2BodyDef type variable ballbd which describes the body.
		ballbd.type = b2_dynamicBody;/// body is dynamic i.e it can move
		/** bodies are at 1.0f along vertical axis and start from 36.0f onwards in horizontal axis at a distance of 10 from each other
		towards the negative x- axis. */		
		ballbd.position.Set(36.0f - 10.0f * i,1.0f );
		spherebody = m_world->CreateBody(&ballbd);///create the body using ballbd
		spherebody->CreateFixture(&ballfd);/// add fixture ballfd defined above to the body
	}
	///7. b) <b>balls on platform above the pulley system near the bottom</b>
	for (int i = 0; i < 2; ++i){
    		b2BodyDef ballbd;/// b2BodyDef type variable ballbd which describes the body.
		ballbd.type = b2_dynamicBody;/// body is dynamic i.e it can move
    ...
    /// 11. Sequence of 5 see-saw systems (see saw + ball + base) in lower right portion 
